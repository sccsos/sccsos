...
```
Pod
├── sccsos (sccsos:0.20.3-slim)    ← 主容器，无 Hermes 嵌入
│   ├── FastAPI Server :8765
│   ├── Workflow Engine
│   └── HermesAdapter (docker-exec)
│
└── hermes-agent (sccsos-hermes:0.20.3)  ← Sidecar，仅 Hermes CLI
...
|------|---------------------|----------------------------------------|
| 镜像 | sccsos:0.20.3 (~600MB) | sccsos:0.20.3-slim (~350MB) + sccsos-hermes:0.20.3 (~120MB) |
| 总大小 | ~600MB | ~470MB |
| 扩缩容 | 绑定 | 可独立 HPA |
| 版本管理 | 同步升级 | 独立升级 |
...
1. 构建 slim 和 hermes 镜像：
   ```bash
   docker build -t sccsos:0.20.3-slim -f Dockerfile.slim .
   docker build -t sccsos-hermes:0.20.3 -f Dockerfile.hermes .
   ```
